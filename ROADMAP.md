...
## Errata RFC prises en compte

Relecture effectuée le 2026-08-25 par rapport à la base d'errata officielle de la RFC Editor (`https://www.rfc-editor.org/errata/rfc<NNNN>`). Seules les errata au statut **Verified** sont considérées comme des corrections normatives à refléter dans cette roadmap ; les errata `Reported`, `Held for Document Update` et `Rejected` sont mentionnées pour information mais ne modifient pas les statuts ci-dessous. Les items de roadmap concernés portent désormais un renvoi `[Errata EID xxxx]`.

| RFC | Errata Verified impactant la roadmap | Statut vis-à-vis de la roadmap |
|---|---|---|
| RFC 8040 | [EID 5255](https://www.rfc-editor.org/errata/eid5255) — §3.5.3, il manque le caractère `=` entre le nom de liste et la valeur de clé en cas de clés multiples dans le path segment | ⚠️ Correction ajoutée à R43 |
| RFC 8040 | [EID 6277](https://www.rfc-editor.org/errata/eid6277) — §4.8.5, la valeur par défaut `last` du paramètre `insert` ne s'applique qu'à la création ; un PUT qui remplace une entrée existante d'une liste `ordered-by user` ne doit pas la déplacer sauf si `insert` est explicitement fourni | ⚠️ Correction ajoutée à R20 |
| RFC 8040 | [EID 6473](https://www.rfc-editor.org/errata/eid6473) — Appendix B.3.2 (exemple `depth`), les listes (`artist`, `song`) doivent être encodées comme des tableaux JSON (`[]`) et non des objets (`{}`), conformément à RFC 7951 | ⚠️ Correction ajoutée à R19/R7 |
| RFC 8040 | [EID 7311](https://www.rfc-editor.org/errata/eid7311) — §7, `missing-element` manquait dans la table de correspondance `error-tag` → code HTTP | ✅ Déjà conforme (la table de la présente roadmap liste `missing-element` → 400) |
| RFC 8040 | [EID 7866](https://www.rfc-editor.org/errata/eid7866) — §3.5.3 et §5.1, le pourcent-encodage des caractères réservés doit aussi référencer la Section 2.2 de RFC 3986 (pas seulement 2.1 et 2.5) | ⚠️ Correction ajoutée à R43 |
| RFC 8040 | [EID 5566](https://www.rfc-editor.org/errata/eid5566) — Appendix B.3.2, une valeur `decimal64` doit être encodée comme une chaîne JSON (`"0.5"`), pas comme un nombre (`0.5`), conformément à RFC 7951 §6.1 | ℹ️ Point d'attention JSON encoding, rattaché à R41 |
| RFC 8040 | Errata `Reported`/`Rejected` non retenues : EID 6215, 7884 (reported), 5565, 5761, 5857, 5858, 6271, 6342, 6362, 6363 (rejected/non vérifiées) | Aucune action — non normatif |
| RFC 8527 | Aucune errata publiée à ce jour (page RFC Editor sans mention "Errata Exist") | ✅ Rien à signaler |
| RFC 8072 | [EID 5131](https://www.rfc-editor.org/errata/eid5131) — §2.2, le `404 Not Found` sur une ressource cible d'un edit inexistante ne s'applique qu'aux opérations `delete`/`move` (et non à toutes les opérations différentes de `create`) | ⚠️ Correction ajoutée à R26 |
| RFC 8639 | [EID 8725](https://www.rfc-editor.org/errata/eid8725) — §2.5.1, la notification de fin de souscription configurée s'appelle `subscription-completed`, et non `subscription-concluded` (nom absent du module YANG) | ⚠️ Correction ajoutée à R33 (notification manquante dans la liste) |
| RFC 8639 | Errata non vérifiées non retenues : EID 8073 (reported), 6211, 8427 (held for document update), 6366 (rejected) | Aucune action — non normatif |
| RFC 8641 | Aucune errata publiée à ce jour | ✅ Rien à signaler |
| RFC 8650 | [EID 6367](https://www.rfc-editor.org/errata/eid6367) — Appendix A.1.3, l'entrée d'un RPC de souscription (`delete-subscription`, et par extension `establish-subscription`/`modify-subscription`/`kill-subscription`/`resync-subscription`) doit être encodée sous un conteneur `input` qualifié par le module (ex. `"ietf-subscribed-notifications:input"`), pas sous le nom du RPC, conformément à RFC 8040 §3.6.1 | ⚠️ Correction ajoutée à R37/R38 |
| RFC 8650 | EID 6369, 6379, 7400 (verified) — corrections purement éditoriales des exemples de l'Appendix A (légende de figure, cohérence id/URI, CRLF manquant) | Aucune action — n'affecte pas le contenu normatif de la roadmap |
| RFC 8071 | Aucune errata publiée à ce jour | ✅ Rien à signaler |
| RFC 8341 | 1 errata au statut `Rejected` (EID 6493, sur la forme des `node-instance-identifier`) | Aucune action |
| RFC 8342 | 1 errata `Verified` (EID 5362) mais purement éditoriale (correction de références croisées vers RFC 7223/7277 au lieu de RFC 8343/8344) | Aucune action |
| RFC 8525 | 1 errata `Held for Document Update` (EID 6484, date de révision initiale du module YANG) — éditoriale | Aucune action |
...
| R2 | [RFC 7519](https://datatracker.ietf.org/doc/html/rfc7519), [RFC 7515](https://datatracker.ietf.org/doc/html/rfc7515), [RFC 7517](https://datatracker.ietf.org/doc/html/rfc7517), [RFC 8725](https://datatracker.ietf.org/doc/html/rfc8725), [RFC 6750](https://datatracker.ietf.org/doc/html/rfc6750) - Identification du client | Le backend valide le JWT reçu dans `Authorization: Bearer`. Validation de la signature JWS, récupération des clés via JWKS, gestion du `kid`, validation de `exp`, `iss`, `aud`, `nbf`, rejet des algorithmes non sûrs. Extraction de l’identité et des groupes pour NACM. Si les tokens sont des Bearer tokens OAuth2, appliquer RFC 6750. | R1 | Conditionnel si JWT | ⛔ Bloqué — `src/jwt.c` (`jwt_bearer_cb`/`jwt_checker_config_key`) n'implémente que la branche `CONFIG_ALG_NONE`, qui configure explicitement `JWT_ALG_NONE` : le jeton n'est donc pas signé/vérifié (pas de JWS, pas de JWKS, pas de `kid`), à l'exact opposé de RFC 8725 §3.1 qui impose de rejeter `alg=none`. La branche `#else` d'un algo réel (`RS256`) est un `#error` non implémenté. `exp`/`iss`/`aud`/`nbf` ne sont pas vérifiés. |
...
| R4 | [RFC 8040 §3.3](https://datatracker.ietf.org/doc/html/rfc8040#section-3.3) - API Resource | Exposer la ressource racine RESTCONF avec les sous-ressources `data` et `operations`. Les ressources de monitoring, notamment `restconf-state`, `capabilities` et `streams`, doivent être exposées selon les fonctionnalités supportées. Valider les URI exactes conformément à RFC 8040 et aux modules YANG de monitoring. | R3 | Obligatoire | 🟡 En cours — `resconf_root_answer()` (`restconf-rooter.c`) expose `{+restconf}` avec `data`, `operations` et `yang-library-version` ; `restconf-state/capabilities` accessible en lecture générique via `/restconf/data/...` grâce au fournisseur opérationnel sysrepo (`sysrepo/plugin.c`) ; `streams` non exposé (dépend de R23) |
| R5 | [RFC 8040 §3.4](https://datatracker.ietf.org/doc/html/rfc8040#section-3.4), [RFC 8342](https://datatracker.ietf.org/doc/html/rfc8342), [RFC 8527](https://datatracker.ietf.org/doc/html/rfc8527) - Datastore Resource | Exposer `{+restconf}/data` comme vue du datastore conceptuel. Si NMDA est supporté, documenter et valider le comportement de `/data` par rapport aux datastores NMDA exposés via `/ds/<datastore>`. Préciser le comportement en lecture, écriture, config, non-config et operational. | R4 | Obligatoire | 🟡 En cours — lecture de `/restconf/data` (`resconf_root_data_answer`) et `/restconf/data/<path>` (`resconf_get_data`) fonctionnelle via `sr_get_data()` ; **écriture non implémentée** (`resconf_update_data()` dans `restconf-rooter.c` est un stub qui `return 0;` sans construire de réponse, quels que soient POST/PUT/PATCH/DELETE) ; `/restconf/ds` ne renvoie que la liste des datastores de la YANG Library, pas une vraie ressource NMDA `/ds/<datastore>` (R27) |
| R6 | [RFC 8040 §4.3](https://datatracker.ietf.org/doc/html/rfc8040#section-4.3) - GET : lecture simple | Lecture d’une ressource de données simple (leaf/container). Support de HEAD, mêmes règles sans corps. Respect de la négociation de contenu et des media types RESTCONF. | R5, R41 | Obligatoire | 🟡 En cours — `resconf_get_data()`/`restconf_send_answer()` fonctionnels (lecture via xpath + `sr_get_data`), HEAD géré (corps omis dans `restconf_send_answer`) ; NACM non appliqué (R8/R29) |
| R7 | [RFC 8040 §4.3](https://datatracker.ietf.org/doc/html/rfc8040#section-4.3), [RFC 7951](https://datatracker.ietf.org/doc/html/rfc7951) - GET : listes/leaf-lists | Lecture de listes et leaf-lists. En XML, la réponse doit contenir un seul élément racine. En JSON, encodage conforme à RFC 7951. | R6 | Obligatoire | 🟡 En cours — `parse_segment()` (`restconf-rooter.c`) construit les prédicats XPath pour listes/leaf-lists (clés multiples, virgules, Errata EID 5255) ; encodage délégué à libyang (`lyd_print_clb`) |
...
| R9 | [RFC 8040 §3.4.1, §3.5.1-2](https://datatracker.ietf.org/doc/html/rfc8040#section-3.5.1), [RFC 9110](https://datatracker.ietf.org/doc/html/rfc9110) - Conditional Requests | Générer `ETag` et `Last-Modified` sur les réponses GET/HEAD. Les validateurs doivent être déterministes et cohérents par ressource/datastore. Supporter `If-Match`, `If-None-Match`, `If-Modified-Since`, `If-Unmodified-Since` lorsque pertinent. Retourner `304 Not Modified` ou `412 Precondition Failed` selon le cas. Voir A11. | R6, R42 | Obligatoire | ⬜ À faire — aucune trace d'`ETag`/`Last-Modified`/`If-Match` dans le code (`struct restconf_ctx`, `restconf.h`, `restconf-error.c`, `restconf-rooter.c`) ; les en-têtes conditionnels ne sont ni parsés (`restconf_header()` ne traite que `accept`/`content-type`) ni produits |
| R10 | [RFC 8040 §4.4.1](https://datatracker.ietf.org/doc/html/rfc8040#section-4.4.1) - POST : Create Resource Mode | Création d’une ressource enfant via POST sur le parent. Retourner `201 Created` avec header `Location`. Si la ressource existe déjà, retourner `409 Conflict` avec `error-tag=data-exists`. | R5, R41 | Obligatoire | ⬜ À faire — **non implémenté** : POST sur `/restconf/data/...` est routé vers `resconf_update_data()` (`restconf-rooter.c`), qui `return 0;` sans jamais appeler `restconf_send_answer`/`restconf_send_error`/`h2c_send_*` ; aucune réponse HTTP n'est émise |
| R11 | [RFC 8040 §4.4.2](https://datatracker.ietf.org/doc/html/rfc8040#section-4.4.2) - POST : Invoke Operation Mode (action) | Invocation d’une action YANG définie sur un data resource via POST. Retourner `200 OK` avec corps si sortie, sinon `204 No Content`. Appliquer NACM `exec`. | R5, R41, R29 | Obligatoire | 🟡 En cours — les RPC top-level (`/restconf/operations/<rpc>`) passent par `resconf_rpc()` (`sr_rpc_send_tree`), mais les actions liées à une data resource (POST sous `/restconf/data/...`) tombent dans le même stub `resconf_update_data()` que R10/R12/R13/R14, donc non implémentées ; **bug mémoire** dans `ly_in_new_evbuffer()` (`libyang.c`) : condition `if (str) return LY_EMEM;` inversée (devrait être `if (!str)`) et `in[len] = '\0';` écrit hors-bornes sur `struct ly_in **` au lieu de `str[len]` — tout POST avec corps d'entrée (RPC/action) passe par ce chemin et est à corriger avant tout test |
| R12 | [RFC 8040 §4.5](https://datatracker.ietf.org/doc/html/rfc8040#section-4.5) - Data Resource : PUT | Création ou remplacement complet d’une ressource de données. Respect des préconditions HTTP. Retourner `201 Created` si création, `204 No Content` si remplacement. | R5, R41, R42 | Obligatoire | ⬜ À faire — **non implémenté**, même stub `resconf_update_data()` que R10 (aucune réponse émise) |
| R13 | [RFC 8040 §4.7](https://datatracker.ietf.org/doc/html/rfc8040#section-4.7) - Data Resource : DELETE | Suppression d’une ressource existante. Retourner `204 No Content`. Gérer accès, ressource absente, conflits, verrous et préconditions. | R5, R42 | Obligatoire | ⬜ À faire — **non implémenté**, même stub `resconf_update_data()` que R10 (aucune réponse émise) |
| R14 | [RFC 8040 §4.6](https://datatracker.ietf.org/doc/html/rfc8040#section-4.6) - PATCH (plain patch) | Support du plain patch RESTCONF. Le plain patch peut fusionner et/ou créer des sous-ressources. Ne pas retourner systématiquement `404` si la cible n’existe pas. Retourner `200 OK` ou `204 No Content` selon le cas. | R6, R12, R41 | Obligatoire | ⬜ À faire — **non implémenté**, même stub `resconf_update_data()` que R10 (aucune réponse émise) |
| R15 | [RFC 8040 §3.6](https://datatracker.ietf.org/doc/html/rfc8040#section-3.6) - Operations Resource | Exposer la ressource `operations`. Supporter `GET {+restconf}/operations` pour découvrir les RPC disponibles, `OPTIONS` pour les méthodes autorisées, et `POST {+restconf}/operations/<rpc>` pour invoquer un RPC. Retourner `200 OK` avec corps si sortie, sinon `204 No Content`. Appliquer NACM `exec`. | R4, R40, R29 | Obligatoire | 🟡 En cours — `resconf_root_operations_answer()` liste les RPC compilés de chaque module implementé ; `resconf_rpc()` invoque via `sr_rpc_send_tree` et gère sortie/`204`; OPTIONS câblé ; mais parsing d'un corps d'entrée affecté par le bug `ly_in_new_evbuffer()` (voir R11) et NACM `exec` non appliqué (R29) |
...
| R22 | [RFC 8040 §7](https://datatracker.ietf.org/doc/html/rfc8040#section-7), [RFC 9110](https://datatracker.ietf.org/doc/html/rfc9110) - Error Reporting | Modèle d’erreurs `ietf-restconf:errors`. Centraliser le mapping `error-tag` → code HTTP → enveloppe JSON/XML. Inclure les headers requis : `WWW-Authenticate` pour `401`, `Allow` pour `405`. Gérer `406`, `415`, `412`. Les erreurs après établissement d’un flux SSE ne peuvent plus être renvoyées via une enveloppe RESTCONF classique. | Plusieurs | Obligatoire | 🟡 En cours — `restconf_send_error()`/`rest_err_create()` (`restconf-error.c`) construisent l'enveloppe `ietf-restconf:errors` en JSON/XML via libyang, avec un traducteur `sr_err_to_netconf_error()` qui mappe de nombreux messages sysrepo/libyang vers des `error-tag` NETCONF ; `restconf_set_status()` couvre la majorité des `error-tag` du tableau §7 (avec `WWW-Authenticate`/`Allow`) mais **`malformed-message` n'a pas de `case` dans le switch** (longueur 17 collisionne avec `missing-attribute`/`unknown-attribute`/`unknown-namespace`/`rollback-failed`, `rest_assert(0)` sinon) et aucun code n'émet effectivement ce tag ; à vérifier avant de clôturer R22 |
...
| R26 | [RFC 8072](https://datatracker.ietf.org/doc/html/rfc8072) - YANG Patch Media Type | Édition de plusieurs sous-ressources en une seule requête PATCH avec `application/yang-patch+json` ou `application/yang-patch+xml`. Supporter `create`, `delete`, `insert`, `merge`, `move`, `remove`, `replace`. Répondre avec `ietf-yang-patch:yang-patch-status`, incluant `global-errors` ou `edit-status`. Ne pas réutiliser brutalement `ietf-restconf:errors`. **[Errata EID 5131]** Un `404 Not Found` sur un edit ne doit être renvoyé que si l'opération est `delete` ou `move` et que la ressource cible n'existe pas ; ce n'est pas le cas pour toutes les opérations différentes de `create` (le texte initial de la RFC était imprécis sur ce point). À noter également que la RFC ne précise pas explicitement le code attendu pour un `create` sur une ressource déjà existante (à traiter comme `400`/`data-exists` par cohérence avec R10/R14). | R14, R41 | Conditionnel si YANG Patch | ⬜ À valider |
...
| R29 | [RFC 8341](https://datatracker.ietf.org/doc/html/rfc8341) - NACM | Contrôle d’accès NACM appliqué à toutes les opérations RESTCONF : GET, HEAD, POST, PUT, PATCH, DELETE, RPC, actions, notifications, streams et souscriptions. Mapper l’identité JWT vers un utilisateur/groupe NACM. Positionner l’identité de session sysrepo avant tout accès aux données. Appliquer les règles `read`, `write`, `exec`. Prévoir des règles par défaut sûres. | R2 | Obligatoire | ⬜ À faire — `restconf_start_session()` (`restconf.c`) appelle `sr_session_set_user(ctx->session, ctx->username)` ; **à vérifier** : d'après la doc sysrepo, `sr_session_set_user()` change l'utilisateur effectif de session (permissions fichiers, SUPERUSER requis), distinct de `sr_nacm_set_user()` (`<sysrepo/netconf_acm.h>`) qui active réellement NACM pour la session ; si c'est confirmé, l'identité JWT n'est pas propagée au moteur NACM et aucune règle NACM ne s'applique actuellement | 
...
| R33 | [RFC 8639 §2.4.5-2.4.6](https://datatracker.ietf.org/doc/html/rfc8639#section-2.4.5) - Notifications de cycle de vie | Notifications de cycle de vie : `subscription-started` si applicable, `subscription-terminated`, `subscription-modified`, `subscription-resumed`, `subscription-suspended`, `subscription-completed` (souscriptions configurées atteignant leur `stop-time`, cf. R47), `replay-completed` si pertinent. **[Errata EID 8725]** Le nom normatif de cette dernière notification est bien `subscription-completed` (définie dans le module YANG et en §2.7.6) ; la Figure 8 et le texte associé de la §2.5.1 de la RFC 8639 mentionnaient à tort `subscription-concluded`, qui n'existe pas dans le modèle. | R31 | Conditionnel si souscriptions | ⬜ À valider |
...
| R37 | [RFC 8650 §2.1-2.2](https://datatracker.ietf.org/doc/html/rfc8650#section-2.1) - Binding RESTCONF : établissement | `establish-subscription` invoqué via POST sur `/operations`. Valider le code de succès attendu selon RFC 8650 : `200 OK` avec sortie, ou `201 Created` + `Location` si applicable. Retourner l’identifiant de souscription et/ou l’URI SSE dédiée. **[Errata EID 6367]** Les paramètres d'entrée du RPC doivent être encodés sous un conteneur `input` qualifié par le nom du module (ex. `"ietf-subscribed-notifications:input"`), conformément à RFC 8040 §3.6.1, et non sous le nom du RPC lui-même. | R31, R15 | Conditionnel si souscriptions RESTCONF | ⬜ À valider |
| R38 | [RFC 8650 §2.3](https://datatracker.ietf.org/doc/html/rfc8650#section-2.3) - Binding RESTCONF : gestion | `modify-subscription`, `delete-subscription`, `kill-subscription` via POST sur `/operations`, cohérent avec R32. **[Errata EID 6367]** Même remarque que R37 : les paramètres d'entrée doivent être encodés sous `"ietf-subscribed-notifications:input"` (RFC 8040 §3.6.1), pas sous le nom du RPC. | R32, R37 | Conditionnel si souscriptions RESTCONF | ⬜ À valider |
...
| R41 | [RFC 8040](https://datatracker.ietf.org/doc/html/rfc8040), [RFC 7951](https://datatracker.ietf.org/doc/html/rfc7951) - Media types et négociation de contenu | Supporter les media types RESTCONF `application/yang-data+json` et `application/yang-data+xml` pour une conformité stricte. Gérer `Accept`, `Content-Type`, `406 Not Acceptable`, `415 Unsupported Media Type`. Renvoyer les erreurs dans le même media type que la requête lorsque possible. | R1 | Obligatoire | 🟡 En cours — `restconf-accept.c` (`restconf_parse_accept`/`restconf_parse_content`/`restconf_check_accept`) gère `Accept`/`Content-Type`, retombe sur JSON par défaut, et route vers l'erreur `invalid-value`→`406`/`415` via `restconf_set_status()` (détection par contenu du message, cf. R22) ; erreurs répondues dans le format négocié (`restconf_get_format`) |
...
| R45 | [RFC 8040 §3.8](https://datatracker.ietf.org/doc/html/rfc8040#section-3.8) - RESTCONF capabilities | Exposer les capacités RESTCONF dans `restconf-state/capabilities`. Annoncer les capacités supportées : `with-defaults`, `depth`, `fields`, `filter`, YANG Patch, NMDA, souscriptions, YANG-Push, etc. | R4, R16 | Obligatoire | 🟡 En cours — `plugin_capabilities_cb()` (`sysrepo/plugin.c`) fournit en opérationnel `with-defaults` (mode figé à la compilation via `CONFIG_CAPABILITY_*`), `depth`, `fields`, `filter`, `replay`, `with-origin`, `start-time`, `stop-time` ; capacités annoncées sans lien avec le support réel (`fields`/`filter` annoncées alors que non fonctionnelles, cf. R19 ; pas de capacité YANG Patch/NMDA/souscriptions/YANG-Push, cohérent avec leur non-implémentation) |
...
| A7 | libjwt | Ne fait pas le fetch JWKS lui-même : coder la récupération HTTP du JWKS, un cache indexé par `kid`, et le rafraîchissement périodique/à rotation de clé. | R2 | ⬜ À faire — confirmé : `jwt.c` ne contient aucun fetch HTTP JWKS ; seule la branche `CONFIG_ALG_NONE` (`alg=none`) est câblée, la branche clé réelle est un `#error` |
| A8 | libjwt | Pas de garantie de thread-safety d’un même contexte `jwt_t` réutilisé entre threads workers — créer/valider un token par requête plutôt que partager un contexte entre threads concurrents. | R2 | ⬜ À faire — non applicable pour l'instant (pas de vérification de signature) ; `jwt_bearer_token_get_name()` crée bien un `jwt_checker_t` par appel (`jwt_checker_auto_t`), point positif à conserver le jour où RS256 sera implémenté |
| A9 | libsysrepo (NACM) | Mapper l’identité extraite du JWT vers l’utilisateur NACM via `sr_session_set_orig_name()` avant tout appel de données, sinon les règles NACM ne s’appliquent pas au bon compte. | R2, R29 | ⬜ À faire — le code appelle `sr_session_set_user()` (`restconf.c`), pas `sr_nacm_set_user()`/`sr_session_set_orig_name()` ; à confirmer auprès de la doc sysrepo installée mais l'API utilisée semble être celle de l'utilisateur effectif de session (permissions fichiers), pas celle qui active NACM (voir R29) |
| A10 | Gestion d’erreurs (R22) | Centraliser le mapping `error-tag` → code HTTP → enveloppe JSON/XML `ietf-restconf:errors` dans une fonction/table unique en C, appelée par tous les handlers. | R22 | 🟡 En cours — largement avancé depuis la dernière relecture : `restconf_set_status()` + `rest_err_create()`/`restconf_send_error()` (`restconf-error.c`) centralisent désormais le mapping complet `error-tag`→HTTP et l'enveloppe `ietf-restconf:errors` JSON/XML via libyang, avec un traducteur additionnel des messages sysrepo/libyang bruts vers des `error-tag` NETCONF (`sr_err_to_netconf_error`) ; lacune restante : `malformed-message` absent du switch (voir R22) |
...
| A12 | Parsing URI | Le décodage du `api-path` doit être fait avant toute construction de XPath libyang — ne jamais concaténer directement le path brut dans un `lyd_new_path` / `lys_find_path`. | R6, R7, R8, R9, R10, R11, R12, R13, R14, R43 | 🟡 En cours — respecté pour la lecture : `parse_segment()` décode (`curl_unescape`) avant `lys_find_path`/construction du prédicat XPath ; sans objet pour les segments écrits par R10/R12/R13/R14 tant qu'ils restent non implémentés |
...
| A25 | sécurité JWT | Whitelister les algorithmes JWT, refuser `none`, valider `aud`, `iss`, `exp`, `nbf`, `kid`, et prévoir une politique de cache JWKS sécurisée. | R2, R44 | ⛔ Bloqué — à l'inverse de l'exigence, la seule voie active du code (`CONFIG_ALG_NONE` dans `jwt.c`) configure expressément `JWT_ALG_NONE` comme algorithme autorisé ; `aud`/`iss`/`exp`/`nbf`/`kid` non vérifiés ; pas de cache JWKS (voir A7) |
...
**Note errata :** la présence de `missing-element` → 400 ci-dessous est conforme à [EID 7311](https://www.rfc-editor.org/errata/eid7311) (Verified), qui a ajouté cette ligne manquante à la table normative de RFC 8040 §7. Le choix `data-missing` → 409 ci-dessous reste également conforme : une proposition de le faire passer à 404 ([EID 5565](https://www.rfc-editor.org/errata/eid5565)) a été rejetée par le groupe de travail.
