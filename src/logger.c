...
void rc_log_init(rc_log_target_t target, int runtime_level) {
    ...
    /* On ne peut pas demander plus de verbosité (niveau plus bas) 
     * que ce qui a été compilé. On clamp au niveau compilé. */
    ...
    if (g_log_target == RC_LOG_TARGET_SYSLOG) {
        openlog("restconf-server", LOG_PID | LOG_NDELAY, LOG_DAEMON);
    }
}
...
void rc_log_print(int level, const char *file, int line, const char *fmt, ...) {
    ...
    if (g_log_target == RC_LOG_TARGET_SYSLOG) {
        ...
        syslog(level_to_syslog[level], "[%s] %s:%d - %s", level_strings[level], base, line, buf);
    } else {
        time_t t = time(NULL);
        struct tm tm;
        localtime_r(&t, &tm);
        ...
        fprintf(stderr, "%s [%-5s] %s:%d - ", time_buf, level_strings[level], base, line);
