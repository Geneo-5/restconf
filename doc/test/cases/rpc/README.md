...
Vérifier que le module est chargé et que ses RPC, actions et notifications sont accessibles :
```bash
# Vérifier les RPC disponibles
GET /restconf/operations/rt:*
# Vérifier les streams disponibles  
GET /restconf/streams/rt:*
```
...
| Cas de test | Applicable | Structure YANG | URI/Opération |
|-------------|------------|----------------|---------------|
| TC-5-001 à TC-5-003 | ✅ | RPC discovery | `/restconf/operations` |
| TC-5-004 | ✅ | RPC sans param | `POST /restconf/operations/rt:get-system-status` |
| TC-5-005 | ✅ | RPC avec params | `POST /restconf/operations/rt:configure-device` |
| TC-5-006 | ✅ | Param obligatoire | `POST /restconf/operations/rt:create-resource` (sans name) |
| TC-5-007 | ✅ | Param invalide | `POST /restconf/operations/rt:process-data` (type invalide) |
| TC-5-008 | ✅ | Param supplémentaire | `POST /restconf/operations/rt:configure-device` (+ champ inconnu) |
| TC-5-009 | ✅ | Valeurs limites | `POST /restconf/operations/rt:set-operation-mode` (enum) |
| TC-5-010 | ✅ | RPC avec output | `POST /restconf/operations/rt:configure-device` → vérif result |
| TC-5-011 | ✅ | RPC sans output | `POST /restconf/operations/rt:trigger-event` |
| TC-5-012 | ✅ | Erreur métier | `POST /restconf/operations/rt:create-resource` (nom existant) |
| TC-5-013 | ✅ | Erreur interne | Déclencher erreur serveur contrôlée |
| TC-5-014 | ✅ | Timeout | RPC long sur `/restconf/operations/rt:configure-device` |
| TC-5-015 à TC-5-019 | ⚠️ | Actions YANG | Actions attachées à `/rt:restconf-test/rt:device-management` |
| TC-5-020 à TC-5-022 | ✅ | Streams discovery | `/restconf/streams` |
| TC-5-023 | ✅ | Réception notification | Streams `rt:system-startup`, `rt:event-notification` |
| TC-5-024 | ✅ | Plusieurs notifications | Ordre préservé |
| TC-5-025 | ✅ | Notification vide | Pas de notification invalide |
| TC-5-026 | ✅ | Notification avec données | `rt:threshold-exceeded` (données structurées) |
| TC-5-027 à TC-5-030 | ✅ | Déconnexion/Reconnexion | Gestion du stream |
| TC-5-031 à TC-5-034 | ✅ | Sécurité | Accès authentifié/autorisé aux RPC |
| TC-5-035 à TC-5-040 | ✅ | Robustesse | Payloads volumineux/vides/invalides |
...
**Module à utiliser** : `restconf-test.yang` contient 6 RPCs, 3 Actions YANG et 4 Notifications pour couvrir tous les cas de test.
...
**Exemples concrets avec restconf-test.yang** :
- RPC : `rt:get-system-status`, `rt:configure-device`, `rt:create-resource`, `rt:set-operation-mode`, `rt:process-data`, `rt:trigger-event`
- Actions : `rt:reset`, `rt:test-connection`, `rt:reboot` (attachées à `/rt:restconf-test/rt:device-management`)
- Notifications : `rt:system-startup`, `rt:threshold-exceeded`, `rt:event-notification`, `rt:system-alert`
...
- présence des RPC `rt:*` du module `restconf-test.yang`

### Critères d'acceptation

La liste des RPC est accessible et inclut au minimum :
