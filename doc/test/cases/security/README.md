...
**Les tests de cette phase peuvent utiliser `restconf-test.yang`** (namespace: `urn:restconf:test`, prefix: `rt`) comme module de référence pour valider les contrôles d'accès (NACM).

Les tests de sécurité sont indépendants des modules YANG spécifiques, mais `restconf-test.yang` peut être utilisé pour valider que les contrôles d'accès fonctionnent correctement.
